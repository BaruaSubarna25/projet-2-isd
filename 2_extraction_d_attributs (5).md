...
def my_foreground_filter(img):
    mfg = scipy.ndimage.gaussian_filter(
        invert_if_light_background(
            foreground_redness_filter(img, theta=0.5)
        ).astype(float),
        sigma=1
    )
    mfg = mfg > 0.5
...
Zones claires = valeur R − G élevée, le pixel est rouge

Zones sombres = valeur R − G faible ou négative, le pixel est vert, bleu, ou neutre

Le fond varie selon la couleur de l'arrière-plan de chaque image originale :

Un fond blanc (RGB élevés) donne R−G = 0, gris moyen

Un fond gris clair donne aussi R−G = 0, gris moyen

Un fond avec une teinte verdâtre donne R−G < 0, sombre

Un fond avec une teinte rougeâtre donne R−G > 0, clair
...
df_features = pd.DataFrame({
    ...
    'class': clean_images.index.map(
        lambda name: 1 if name[0] == 'a' else -1
    )
})
df_features
```
...
---
# VOTRE CODE ICI
df_colors = clean_images.apply(get_colors)
```
...
df_features_large['class'] = df_class
...
Le meilleur nombre d’attributs est d’environ 4 à 5, car c’est à ce niveau que la performance sur le jeu de test est maximale.
Ajouter davantage d’attributs n’améliore pas les performances et peut introduire du bruit, ce qui conduit à du sur-apprentissage.
...
---
# VOTRE CODE ICI
col_selected = ranked_columns[0:4]  # ou le nombre optimal que vous avez choisi
df_features_final = df_features_large[col_selected].copy()
df_features_final['class'] = df_features_large['class']
...
---
# VOTRE CODE ICI
performances.loc[3] = ["Analyse de variance univariée", p_tr, s_tr, p_te, s_te]
performances.style.format(precision=2).background_gradient(cmap='Blues')
```
...
---
# VOTRE CODE ICI
performances.loc[3] = ["Meilleure paire(multivariée)", tr_best, tr_std, best_perf, std_perf]
performances.style.format(precision=2).background_gradient(cmap='Blues')
```
...
+++ {"deletable": false, "nbgrader": {"cell_type": "code", "checksum": "e652b379362bfa03cf695be9d9b10598", "grade": true, "grade_id": "cell-da9ce91479dfe562", "locked": false, "points": 0, "schema_version": 3, "solution": true, "task": false}}

La paire d’attributs surface, C donne les meilleures performances en analyse multi-variée. 

Cependant, cette approche n’est pas nécessaire car elle n’améliore pas les résultats par rapport à une sélection univariée, qui reste plus efficace et plus simple.
...
Nous avons appris l’importance du prétraitement des images, notamment l’extraction de l’avant-plan, le recentrage et le recadrage, qui permettent d’obtenir des données plus propres et exploitables.

Nous avons également compris que la qualité des attributs (features) est essentielle pour les performances d’un modèle. Les attributs liés à la couleur et à la forme permettent de représenter efficacement les images sous forme de données numériques.

L’étape de normalisation s’est révélée indispensable pour garantir que toutes les variables aient le même poids dans le modèle, en particulier pour KNN qui repose sur des distances.

Enfin, nous avons observé que :

Ajouter trop d’attributs peut dégrader les performances (overfitting),

Une sélection d’attributs pertinente améliore les résultats,

L’analyse univariée est souvent suffisante, tandis que l’analyse multi-variée n’apporte pas toujours de gain.
